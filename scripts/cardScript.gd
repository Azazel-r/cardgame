...
func flipCard(secs : float) -> void:
	if !back:
		...
		tweenScale.tween_property($cardArea, "scale", gs.cardScale * Vector2(0.5,1), secs/2).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
		tweenScale.tween_property($cardArea, "scale", gs.cardScale, secs/2).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
		addSkew = 0
		back = true
	else:
		tweenTurn = create_tween()
...
func hoverAnimation(zindexchange := 0) -> void:
	tween = create_tween()
	tween.tween_property($cardArea, "scale", gs.cardScale * Vector2(1.2,1.2), 0.15).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
...
func resetHover(zindexchange := 0) -> void:
	...
	tween.parallel().tween_property($cardArea, "scale", gs.cardScale, 0.15).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
	z_index -= zindexchange
...
func resetHoverNoEase(zindexchange := 0) -> void:
	...
	$cardArea.scale = gs.cardScale * Vector2(1,1)
	z_index -= zindexchange
